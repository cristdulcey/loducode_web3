...
class ContractLand(ABC):  # pylint: disable=R0904

    def __init__(self, bsc: str = settings.CONTRACT_LAND_NET, abi: str = json.loads(settings.CONTRACT_LAND_ABI),
                 address_contract: str = settings.CONTRACT_LAND_ADDRESS,
                 chain_id: int = settings.CONTRACT_LAND_CHAIN_ID):
        self.bsc = bsc
        self.abi = abi
    ...
    def mint(self, address: str, address_owner: str = settings.CONTRACT_LAND_ADDRESS_OWNER,
             secret_owner: str = settings.CONTRACT_LAND_SECRET_OWNER):
        web3 = Web3(Web3.HTTPProvider(self.bsc))
        _contract = web3.eth.contract(address=self.address_contract, abi=self.abi)
        response = {}
        try:
    ...
    def start_pre_sale(self, start=True, address_owner: str = settings.CONTRACT_LAND_ADDRESS_OWNER,
                       secret_owner: str = settings.CONTRACT_LAND_SECRET_OWNER):
        web3 = Web3(Web3.HTTPProvider(self.bsc))
        _contract = web3.eth.contract(address=self.address_contract, abi=self.abi)
        response = {}
    ...
    def start_sale(self, address_owner: str = settings.CONTRACT_LAND_ADDRESS_OWNER,
                   secret_owner: str = settings.CONTRACT_LAND_SECRET_OWNER):
        web3 = Web3(Web3.HTTPProvider(self.bsc))
        _contract = web3.eth.contract(address=self.address_contract, abi=self.abi)
        response = {}
    ...
    def pause(self, address_owner: str = settings.CONTRACT_LAND_ADDRESS_OWNER,
              secret_owner: str = settings.CONTRACT_LAND_SECRET_OWNER, pause_status: bool = True):
        web3 = Web3(Web3.HTTPProvider(self.bsc))
        _contract = web3.eth.contract(address=self.address_contract, abi=self.abi)
        response = {}
        try:
    ...
    def withdraw_all(self, address_owner: str = settings.CONTRACT_LAND_ADDRESS_OWNER,
                     secret_owner: str = settings.CONTRACT_LAND_SECRET_OWNER):
        web3 = Web3(Web3.HTTPProvider(self.bsc))
        _contract = web3.eth.contract(address=self.address_contract, abi=self.abi)
        response = {}
    ...
    def update_base_extension(self, prefix: str = '', address_owner: str = settings.CONTRACT_LAND_ADDRESS_OWNER,
                              secret_owner: str = settings.CONTRACT_LAND_SECRET_OWNER):
        web3 = Web3(Web3.HTTPProvider(self.bsc))
        _contract = web3.eth.contract(address=self.address_contract, abi=self.abi)
        response = {}
    ...
    def update_max_mint_ammount(self, max_mint: int = 1000, address_owner: str = settings.CONTRACT_LAND_ADDRESS_OWNER,
                                secret_owner: str = settings.CONTRACT_LAND_SECRET_OWNER):
        web3 = Web3(Web3.HTTPProvider(self.bsc))
        _contract = web3.eth.contract(address=self.address_contract, abi=self.abi)
        response = {}
    ...
    def burn(self, token_id: int, address_owner: str = settings.CONTRACT_LAND_ADDRESS_OWNER,
             secret_owner: str = settings.CONTRACT_LAND_SECRET_OWNER):
        web3 = Web3(Web3.HTTPProvider(self.bsc))
        _contract = web3.eth.contract(address=self.address_contract, abi=self.abi)
        response = 0
    ...
    def update_token_price(self, new_value: float, address_owner: str = settings.CONTRACT_LAND_ADDRESS_OWNER,
                           secret_owner: str = settings.CONTRACT_LAND_SECRET_OWNER):
        web3 = Web3(Web3.HTTPProvider(self.bsc))
        _contract = web3.eth.contract(address=self.address_contract, abi=self.abi)
        response = 0
    ...
    def transfer_contract(self, new_owner: str, address_owner: str = settings.CONTRACT_LAND_ADDRESS_OWNER,
                          secret_owner: str = settings.CONTRACT_LAND_SECRET_OWNER):
        web3 = Web3(Web3.HTTPProvider(self.bsc))
        _contract = web3.eth.contract(address=self.address_contract, abi=self.abi)
        response = 0
